...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/30 12:37:46 by fde-fede          #+#    #+#             */
...
t_instruction	*rotate_a(t_state *stt, size_t min[2], size_t min_mvt[2])
{
	...
	if (min[0] <= min[1] && min_mvt[0] <= stt->st_b->size / 2)
		instr = add_instruction(&stt->instructions, "rr");
	else if (min[0] < min[1] && min_mvt[0] > stt->st_b->size / 2)
		instr = add_instruction(&stt->instructions, "ra");
	else if (min_mvt[1] > stt->st_b->size / 2)
...
t_instruction	*rotate(t_state *stt)
{
	...
	mvt[0] = closer_pos_to_inf(stt->st_a->array[0], stt->st_b);
	min[0] = less_mvt_at_begin(mvt[0], &min_mvt[0], stt->st_a, stt->st_b);
	min[1] = less_mvt_at_end(mvt[0], &min_mvt[1], stt->st_a, stt->st_b);
	if (mvt[0] > stt->st_b->size / 2)
		mvt[1] = stt->st_b->size - mvt[0];
	else
		mvt[1] = mvt[0];
	if (stt->st_a->size > 1 && (min[0] <= mvt[1] || min[1] <= mvt[1]))
		instr = rotate_a(stt, min, min_mvt);
	else if (mvt[0] > stt->st_b->size / 2)
...
int	large_resolve(t_state *states)
{
	...
	while (tmp_state)
	{
		while (tmp_state->st_a->size > 5)
		{
			if (can_pb(tmp_state->st_a, tmp_state->st_b))

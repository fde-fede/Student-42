...
/*   Updated: 2022/10/09 12:28:39 by fde-fede         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_stack *stack_a, t_stack *stack_b)
{
	...
	while (++i < max_size)
	{
		...
		if ((int)(stack_a->size - max_size + i) >= 0)
			write_n_char(max_size_nbr[0] - size_nbr(stack_a->array[stack_a->size
					- max_size + i]), ' ');
		if ((int)(stack_a->size - max_size + i) >= 0)
		...
		if ((int)(stack_b->size - max_size + i) >= 0)
			write_n_char(max_size_nbr[1] - size_nbr(stack_b->array[stack_b->size
					- max_size + i]), ' ');
		if ((int)(stack_b->size - max_size + i) >= 0)

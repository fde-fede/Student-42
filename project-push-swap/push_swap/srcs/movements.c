...
/*   Updated: 2022/07/31 13:11:18 by fde-fede         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_stack *stack_a, t_stack *stack_b)
{
	...
	while (--i > 0 && (stack_a->size - i) < minimal_mvt)
	{
		...
		if ((t[0] > stack_b->size / 2 && t[1] >= (stack_a->size - i)
				&& t[1] < m) || (t[0] > stack_b->size / 2 && t[1]
				< (stack_a->size - i) && (stack_a->size - i) < m)
			|| ((stack_a->size - i) + t[0] < m))
			*min_mvt = t[0];
		if (t[0] > stack_b->size / 2 && t[1] >= (stack_a->size - i) && t[1] < m)
			m = t[1];
		else if (t[0] > stack_b->size / 2 && t[1] < (stack_a->size - i)
			&& (stack_a->size - i) < m)
			m = (stack_a->size - i);
